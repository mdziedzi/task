...
const Table = ({}: TableProps) => {
    ...
    const {data, isLoading, error} = useQuery<TransactionsDto>('transactions', () => api.getTransactions().then(res => res.data))
            ...
            <table>
                ...
                {filteredTransactions?.length > 0 ? filteredTransactions.map(((item) => (
                    <tr key={item.id}>
                        ...
                        <td>
                            <button onClick={() => handleTransactionDelete(item.id)}>Delete</button>
                        </td>
                    </tr>
